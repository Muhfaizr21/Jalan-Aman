...
export default function Hyperspeed({ className = "", speed = 1.6 }: HyperspeedProps) {
  ...
  useEffect(() => {
    ...
    // Traffic colors: Red taillights (left lane) & Cyan/Electric Blue headlights (right lane)
    const leftColors = ["#ef4444", "#f97316", "#dc2626", "#fb7185"];
    const rightColors = ["#00f5ff", "#38bdf8", "#06b6d4", "#a5f3fc"];
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = "rgba(16, 185, 129, 0.35)"; // Emerald glowing guard rail
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ...
      ctx.strokeStyle = "rgba(56, 189, 248, 0.35)"; // Cyan glowing guard rail
      ctx.beginPath();
      ctx.moveTo(centerX + 15, horizonY);
      ctx.lineTo(centerX + width * 0.55, height);
      ctx.stroke();

      // Highway Center Yellow Dashed Line
      ctx.strokeStyle = "rgba(251, 191, 36, 0.6)"; // Amber road marker
      ctx.lineWidth = 2;
      ...
      for (let i = 0; i < streaks.length; i++) {
        ...
        if (y1 > horizonY && y1 < height + 80 && x1 > -150 && x1 < width + 150) {
          ...
          ctx.lineWidth = Math.max(1, (1600 - s.z) / 220) * s.width;
  ...
  return (
    <div className={cn("absolute inset-0 pointer-events-none overflow-hidden", className)}>
      ...
      {/* Subtle top & bottom fade so text on top is crystal clear */}
