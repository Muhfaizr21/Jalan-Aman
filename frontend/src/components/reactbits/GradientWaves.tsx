...
const detailToSteps = (detail: string): number => {
  if (detail === "low") return 26.0;
  if (detail === "high") return 46.0;
  return 34.0;
};
...
void main() {
  ...
  float isSky = step(MAX_DIST - 100.0, dist);
  float t = clamp(uFogDepth / max(dist, 0.001), 0.0, 1.0);
  vec3 body = mix(uWaveColor, uCrestColor, clamp(pos.z * 0.08 + 0.5, 0.0, 1.0));
  vec3 col = mix(uHorizonColor, body, t);
  col = mix(col, uHorizonColor * 0.5, isSky);
  col *= uBrightness;
  col = clamp(col, 0.0, 1.0);

  float alpha = mix(clamp(t * 1.5, 0.0, 1.0), 0.4, isSky) * uOpacity;
  if (uGrain > 0.5) {
...
export default function GradientWaves({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      dpr: Math.min(typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1, 1.0) * 0.85,
    });
    ...
    const loop = (t: number) => {
      ...
      lastRenderTime = t - ((t - lastRenderTime) % frameInterval);
